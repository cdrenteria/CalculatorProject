...
public class CalculatorGUI extends JFrame
{
	...
	public CalculatorGUI()
	{
		...
		percent.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e)
			{
				//String numEntered = String.format("1%f", Double.parseDouble(numShow.getText())*.01);
		...
		Enter.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				numShow.setText(calculate(numShow.getText()));	
			}
			...
			private String calculate(String input) {
				...
				for (int i = 0; i < charArray.length; i++) {
					if (Character.isDigit(charArray[i]) || charArray[i] == '.' || charArray[i] == '%') {
						if (operator.isEmpty()) {	
							if (charArray[i] == '%') {
								firstOperand = String.format("%.2f", Double.parseDouble(firstOperand)*.01);
							} else {
							...
							if (charArray[i] == '%') {
								secondOperand = String.format("%.2f", Double.parseDouble(secondOperand)*.01);
							} else {
				...
				{
				case "+": 
					total = Double.parseDouble(firstOperand) + Double.parseDouble(secondOperand);
				break;
				case "-": total= Double.parseDouble(firstOperand) - Double.parseDouble(secondOperand);
				break;
				case "*": total= Double.parseDouble(firstOperand) * Double.parseDouble(secondOperand);
				break;
				case "/": total= Double.parseDouble(firstOperand) / Double.parseDouble(secondOperand);
